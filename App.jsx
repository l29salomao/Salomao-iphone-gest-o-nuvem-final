...
function AutoInp({value, onChange, onSelect, placeholder, suggestions=[], style={}}){
  ...
  const handleChange = (e)=>{
    ...
    onChange(v);
    if(v.length>0){
      const f = [...new Set(suggestions)].filter(s=>s&&s.toLowerCase().includes(v.toLowerCase())).slice(0,6);
      setFiltered(f);
      setOpen(f.length>0);
    } else {
      setOpen(false);
  ...
  return (
    <div ref={ref} style={{position:"relative",width:"100%"}}>
      <input value={value} onChange={handleChange}
        onFocus={()=>{
          if(value.length>0){
            const f=[...new Set(suggestions)].filter(s=>s&&s.toLowerCase().includes(value.toLowerCase())).slice(0,6);
...
function Registros({data,save}) {
  ...
  // Suggestions extracted from past registros
  const sugestoes = useMemo(()=>({
    clientes: [...new Set(data.registros.map(r=>r.cliente).filter(Boolean))],
    cidades:  [...new Set(data.registros.map(r=>r.cidade).filter(Boolean))],
    aparelhos:[...new Set(data.registros.map(r=>r.aparelho).filter(Boolean))],
    servicos: [...new Set(data.registros.map(r=>r.servico).filter(Boolean))],
    pecas:    [...new Set(data.registros.map(r=>r.peca).filter(Boolean))],
  }),[data.registros]);
