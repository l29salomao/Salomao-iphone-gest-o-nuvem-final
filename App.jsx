...
// ── AUTOCOMPLETE INPUT ────────────────────────────────────────────────
function AutoInp({value, onChange, onSelect, placeholder, suggestions=[], style={}}){
  ...
  const handleChange = (e)=>{
    ...
    onChange(v);
    if(v.length>0){
      const seen=new Set(); const f=suggestions.filter(s=>{if(!s)return false;const k=s.toLowerCase().trim();if(!k.includes(v.toLowerCase()))return false;if(seen.has(k))return false;seen.add(k);return true;}).slice(0,6);
      setFiltered(f);
      setOpen(f.length>0);
    } else {
      setOpen(false);
  ...
  return (
    <div ref={ref} style={{position:"relative",width:"100%"}}>
      <input value={value} onChange={handleChange}
        onFocus={()=>{
          if(value.length>0){
            const seen2=new Set();const f=suggestions.filter(s=>{if(!s)return false;const k=s.toLowerCase().trim();if(!k.includes(value.toLowerCase()))return false;if(seen2.has(k))return false;seen2.add(k);return true;}).slice(0,6);
      ...
      {open && filtered.length>0 && (
        <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:"#0f0f0f",border:"1px solid var(--gold-glow)",borderRadius:10,zIndex:999,overflow:"hidden",boxShadow:"0 8px 24px rgba(0,0,0,.8)"}}>
          {filtered.map((s,i)=>(
            <div key={i} onClick={()=>handleSelect(s)}
              style={{padding:"9px 12px",fontSize:12,color:"var(--text)",cursor:"pointer",borderBottom:i<filtered.length-1?"1px solid #1a1408":"none",fontFamily:"var(--mono)"}}
...
function Registros({data,save}) {
  ...
  // Suggestions extracted from past registros — deduplicated case-insensitive
  const dedup = (arr) => {
    const seen = new Set();
    return arr.filter(v=>{ const k=v.toLowerCase().trim(); if(seen.has(k))return false; seen.add(k); return true; });
  };
  const sugestoes = useMemo(()=>({
    clientes: dedup(data.registros.map(r=>r.cliente).filter(Boolean)),
    cidades:  dedup(data.registros.map(r=>r.cidade).filter(Boolean)),
    aparelhos:dedup(data.registros.map(r=>r.aparelho).filter(Boolean)),
    servicos: dedup(data.registros.map(r=>r.servico).filter(Boolean)),
    pecas:    dedup(data.registros.map(r=>r.peca).filter(Boolean)),
  }),[data.registros]);

  // When client is selected, autofill cidade, origem, tipoCliente
  const onClienteSelect = (nome) => {
    const registrosCliente = data.registros.filter(r=>r.cliente===nome);
    if(registrosCliente.length>0){
      const ultimo = registrosCliente[registrosCliente.length-1];
      setForm(f=>({...f,
        cliente: nome,
        cidade: ultimo.cidade || f.cidade,
        origem: ultimo.origem || f.origem,
        tipoCliente: "Antigo",
      }));
    }
  };
      ...
      {modal&&(
        <Modal title="➕ Novo Registro" onClose={()=>setModal(false)} color="gold">
          <div style={{display:"flex",flexDirection:"column",gap:9}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div><Lbl>Cliente</Lbl>
                <AutoInp value={form.cliente}
                  onChange={v=>setForm(f=>({...f,cliente:v}))}
                  onSelect={onClienteSelect}
                  placeholder="Nome do cliente"
                  suggestions={sugestoes.clientes}/>
              </div>
              <div><Lbl>Cidade</Lbl>
                <AutoInp value={form.cidade}
                  onChange={v=>setForm(f=>({...f,cidade:v}))}
                  placeholder="Ex: BC"
                  suggestions={sugestoes.cidades}/>
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div><Lbl>Aparelho</Lbl>
                <AutoInp value={form.aparelho}
                  onChange={v=>setForm(f=>({...f,aparelho:v}))}
                  placeholder="Ex: iPhone 14"
                  suggestions={sugestoes.aparelhos}/>
              </div>
              <div><Lbl>Serviço</Lbl>
                <AutoInp value={form.servico}
                  onChange={v=>setForm(f=>({...f,servico:v}))}
                  placeholder="Ex: Tela"
                  suggestions={sugestoes.servicos}/>
              </div>
            </div>
            <div><Lbl>Peça utilizada</Lbl>
                <AutoInp value={form.peca}
                  onChange={v=>setForm(f=>({...f,peca:v}))}
                  placeholder="Opcional..."
                  suggestions={sugestoes.pecas}/>
              </div>
...
function Caixa({data}) {
  ...
  const porDia = useMemo(()=>{
    ...
    return Object.values(map).sort((a,b)=>parseDate(a.data)-parseDate(b.data));
  },[regFiltro]);
            ...
            {porDia.map(d=>{
              ...
              return(
                <div key={d.data} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                  ...
                  <span style={{fontSize:7,color:"#fff",textAlign:"center",whiteSpace:"nowrap",fontWeight:600,textShadow:"0 1px 3px rgba(0,0,0,.8)"}}>{d.data.slice(0,5)}</span>
                </div>
              );
            })}
          </div>
